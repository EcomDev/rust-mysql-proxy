        ...
        loop {
            match &mut this.current_future {
                Some(future) => match future.as_mut().poll(cx) {
                    ...
                    Poll::Ready((event, state)) => {
                        let mut retry_fetch = |state: ConnectionState| {
                            let processor = (this.command_processor)(Command::fetch(), state);
                ...
                None => {
                    ...
                    let processor = (this.command_processor)(command, std::mem::take(this.state));
...
mod tests {
    ...
    async fn modify_state(command: Command, state: ConnectionState) -> (Event, ConnectionState) {
        (
            Event::other_error(format!("{:?}", command)),
            match state {
    ...
    async fn fake_result_set_processor(
        command: Command,
        state: ConnectionState,
        ...
        match (command, state) {
            ...
            (Command::Fetch, ConnectionState::Closed) => {
                (Event::ResultRow(vec![Value::Int(1)]), ConnectionState::None)
            }
            (Command::Fetch, ConnectionState::None) => (Event::ResultEnd, ConnectionState::None),
    ...
    async fn it_automatically_fetches_result_set() {
        let connection = Connection::new(
            stream::iter(vec![Command::query("SELECT 1 as one"), Command::close()]),
        ...
        assert_eq!(
            connection.collect::<Vec<_>>().await,
            vec![
                Event::result_set(vec![Column::new("one", TypeHint::Int)]),
                Event::result_row(vec![Value::Int(1)]),
                Event::result_end(),
                Event::closed()
            ]
        );
